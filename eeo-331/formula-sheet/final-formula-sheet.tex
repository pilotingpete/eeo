...
\varepsilon_0 &= 8.854 \times 10^{-14}\ \mathrm{C}^2/(\mathrm{N}\cdot\mathrm{m}^2) \\
c &= 3.00 \times 10^8\ \mathrm{m/s}\\
e &\approx 1.6 \times 10^{-19}\ \text{C}\\
kT &\approx 0.026 \mathrm{eV} \mathrm{at} T = 300 \text{K}\\
h &= 6.62607015 \times 10^{-34} \mathrm{ m^2 kg/s}\\
\varepsilon_{ox} &= (3.9)(8.85 \times 10^{-14}) = 3.45 \times 10^{-13}  \mathrm{F/m^2}\\
\varepsilon_{s} &= (11.7)(8.85 \times 10^{-14}) = 1.04 \times 10^{-12}  \mathrm{F/m^2}\\
E_g &= 1.12 \mathrm{V}\\
n_i &= 1.5 \times 10^{10}\\
\end{align*}
...
Banding: negative voltages shift plot up. For P-Type, $E_F < E_{Fi}$, Accumulation when $V_g < 0$, Depletion when $V_g>0$.

$\phi_{ms} is the work function$
$\phi_{ms} = \phi'_m - \left[x' + \frac{E_g}{2e} + |\phi_{fp}| \right] $,(V) Aluminum Gate\\
$\phi_{ms} = -\left(\frac{E_g}{2e} + |\phi_{fp}| \right) $,(V) n* Polysilicon Gate\\
$\phi_{ms} = \frac{E_g}{2e} - |\phi_{fp}| $,(V) p* Polysilicon Gate\\
$\phi_{Fp} = -\frac{kT}{e}\ln\left(\frac{N_A}{n_i}\right)$, (V)\\
$\phi_s = 2|\phi_{Fp}|$, (V) Surface Potential\\
$x_{dt} = \left[ \frac{4\epsilon_s |\phi_{fp}|}{eN_A}\right]^{1/2}$ (cm)\\
$V_G = V_{ox} + \phi_s + \phi_{ms}$\\
$V_{FB} = V_{ox} + \phi_{ms}$, at flatband, $V_g \equiv V_{FB}$, $\phi_s = 0$\\
$V_{FB} =  \phi_{ms} - \frac{Q'_{ss}}{C_{ox}}$, from above\\
$Q'_{SD}(max) = -eN_Ax_{dt}$\\
$Q'_{SS} = C_{ox}(\phi_{ms}-V_{FB})$, ($C/cm^2$)\\
$V_{TN} = \frac{|Q'_{SD}(max)|}{C_{ox}} + V_{FB} + 2|\phi_{Fp}|$, threshold voltage\\
$\gamma = \frac{\sqrt{2e\epsilon_sN_A}}{C_{ox}}$\\
$V_{TN} = V_{FB} + 2|\phi_{Fp}| +\gamma\sqrt{2|\phi_{Fp}|}$, threshold voltage\\


%%%%%%%%%%%%%%%%%%_ Begin New Section _%%%%%%%%%%%%%%%%%%%
\subsection{Ideal C-V Characteristics}
...
Operating regions
\begin{itemize}
	\item (BE Junction) Forward $V_{BE} > 0$
	\item (BE Junction) Reverse $V_{BE} < 0$
	\item (CB Junction) Forward $V_{CB} < 0$
	\item (CB Junction) Reverse $V_{CB} > 0$
\end{itemize}

\begin{itemize}
	\item Cutoff: BE reverse, CB reverse
	\item Saturation: BE forward, CB forward
	\item Active: BE forward, CB reverse
	\item Inverse Active: BE reverse, CB forward
\end{itemize}

Depletion or enhancement

Depletion mode when $V_{TN} < 0$, enhancement mode when $V_{TN} > 0$

A mosfet is in Saturation when $V_{GS} > V_{TH}$ and $V_{DS} > V_{DS}(sat)$


$V_{bi}(BE) = \frac{kT}{e}\ln{\frac{N_BN_E}{n_i^2}}$\\


\subsection{Implantation}

Add acceptor atoms to increase $V_{TN}$

$\Delta V_{TN} = \frac{eD_I}{C_{ox}}$, number of atoms per cm sq
