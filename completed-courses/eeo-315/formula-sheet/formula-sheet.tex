...
\section{\S1.1 - Semiconductor Materials}

DC variables are denoted by uppercase letters and uppercase subscripts.

\begin{tabular}{ |c||c|c|  }
 \hline
 \multicolumn{3}{|c|}{Semiconductor constants} \\
 \hline
...
\subsection{Intrinsic carrier concentration of Si}

$$n_i = BT^{3/2}e^{\frac{-Eg}{2kT}}$$

Where:
\begin{itemize}
	\item k is Boltzmann’s constant $\SI{86e-6}{eV/K}$
	\item $B$ is a coefficient related to the specific semiconductor material
	\item $Eg$ is the bandgap energy $(eV)$
	\item $T$ is the temperature (K)
	\item $e$, in this context, represents the exponential function
\end{itemize}
	
\subsection{Extrinsic (doped) semiconductors}

$n_i = \SI{1.5e10}{cm^{-3}}$, intrinsic carrier concentration @ 300K for Si

$n_o = \frac{{n^2}_i}{N_a}$, Electron concentration\\
$p_o = \frac{{n^2}_i}{N_d}$, Hole concentration\\

Majority carrier: N-type: electrons, P-type: holes\\

\subsection{Drift and Diffusion Currents}
